...
class ColdStakingFeeTest(NavCoinTestFramework):
    ...
    def run_test(self):
        ...
        while True:
            time.sleep(1)
            fErrFound = "Coinstake tried to move cold staking coins to a non authorised script" in open(self.options.tmpdir + '/node2/devnet/debug.log').read()
            if fErrFound  or self.nodes[2].getblockcount() >= 401:
                break
